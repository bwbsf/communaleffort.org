...
* Confirm the evidence archive exists at `evidence/`, or create it as part of the approved plan before importing source-backed opportunities.
...
3. **Localize source evidence**
   * Run `python3 scripts/localize_evidence.py --from <completed-report-path>` before importing opportunities when the completed report contains source URLs.
   * If validating an existing chapter page, run `python3 scripts/localize_evidence.py --from <chapter-page-path>`.
   * Use `--url` for any additional source URL that is needed to verify a claim but is not present in the report.
   * The script must skip URLs already represented in `evidence/index.yml` when the local note and raw capture still exist.
   * Review the script summary to identify localized, skipped, and failed URLs, and use the printed evidence note paths during validation.
   * Treat failed localization as a validation warning. Do not import an opportunity whose existence, active status, local relevance, or category fit depends only on a failed source unless another localized or durable source supports the same claim.
   * Use `--refresh` only when local evidence is stale, incomplete, disputed, or missing despite a manifest entry.
   * Do not commit `evidence/raw/` or `evidence/reports/`; commit `evidence/index.yml` and relevant `evidence/notes/` changes.

4. **Validate each opportunity before integration**
   ...
   * Do not rely on the report's prose alone; independently inspect localized evidence notes/raw captures or enough live sources to confirm existence, description, local relevance, and fit.

5. **Fail closed when validation is incomplete**
...
6. **Clean research-output formatting**
...
7. **Integrate validated opportunities into the chapter page**
...
9. **Archive fully integrated reports**
...
10. **Regenerate prompts after integration**
...
11. **Commit before the next report**
...
12. **Reset all opportunities for a chapter if results are wrong**
...
13. **Reset one category for a chapter**
...
* `python3 scripts/localize_evidence.py --dry-run --from <completed-report-path>` reports already-localized or localizable source URLs as expected.
* `evidence/index.yml` includes localized source URLs used for imported opportunities.
* Relevant committed evidence notes exist under `evidence/notes/`; raw captures remain ignored under `evidence/raw/`.
